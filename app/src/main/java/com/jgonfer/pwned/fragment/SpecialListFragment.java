...
public class SpecialListFragment extends BaseFragment implements AllBreachedServicesListRequest.OnLoginResponseListener {
    Unbinder unbinder;
    ...
    private boolean mIsSensitive = false;

    public SpecialListFragment() {

    }

    public void setSpecialType(boolean isSensitive) {
    ...
    public List<BreachedServiceDrawerItem> getData() {
        ...
        if (mBreachedServices != null) {
            for (BreachedService breachedService : mBreachedServices) {
                BreachedServiceDrawerItem coItem = new BreachedServiceDrawerItem(breachedService, false, 0);
                data.add(coItem);
            }
        }
    ...
    private void refreshListViewData(boolean shouldReload) {
        mBreachedServices = RealmHelper.getBreachedServicesBySpecialType(mIsSensitive);
        mAdapter = new BreachedServiceDrawerAdapter(mContext, getData(), null, null);
